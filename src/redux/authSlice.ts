...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    loadMsgFromUserProfiles(state, action: PayloadAction<UserModel>) {
      //state.favUserProfiles = action.payload;
      state.msgFromUserProfiles = [...state.msgFromUserProfiles, action.payload];
    },
...
export const toggleFavourite = (itemId: string, userId: string):
  AppThunk => async (dispatch: AppDispatch) => {
    setIsLoading(true);
    axios
...
export const toggleFavUsers = (favUserId: string, userId: string):
  AppThunk => async (dispatch: AppDispatch) => {
    setIsLoading(true);
    axios
...
export const changePassword = (email: string, newPassword: string):
  AppThunk => async (dispatch: AppDispatch) => { };
...
export const sendMsg = (msg: MsgModel, toUserId: string, updateChat: boolean):
  AppThunk => async (
    dispatch: AppDispatch
  ) => {
    axios
...
export const updateUserImages = (photo: PhotoModel, userId: string):
  AppThunk => async (dispatch: AppDispatch) => {
    setIsLoading(true);
    axios
...
export const deleteUserImage = (photo: PhotoModel, userId: string):
  AppThunk => async (dispatch: AppDispatch) => {
    setIsLoading(true);
    axios
...
export const FromUsersMsgs = (currentUser: UserModel, otherUserId: string):
  AppThunk => async (dispatch: AppDispatch) => {
    setIsLoading(true);
    axios.post("users/FromUsersMsgs/" + currentUser._id, { fromUserId: otherUserId })
      .then((chat) => {
        let chatStream = chat.data as ChatStream;
        let otherUserMsgs = currentUser.messages.filter(msg => msg.fromUser === otherUserId);
...
const loadCurrentuserData = (dispatch: AppDispatch, resUserId: string, token: string) => {
  axios
    .get("users/" + resUserId, {
    ...
    .then((res) => {
      ...
      for (var j = 0; j < res.data.msgFromUsers.length; j++) {
        axios
          ...
          .then((res) => {
            dispatch(authSlice.actions.loadMsgFromUserProfiles(res.data as UserModel));
            setIsLoading(false);
          })
          .catch((er) => {
            console.log(er);
